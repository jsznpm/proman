...
import { createPaginator, navChoices, NAV } from "../src/pagination.js";
...
test("navChoices shows Next only on first page, Prev only on last", () => {
  const p = createPaginator(items, 10);
  let nav = navChoices(p, { exit: { label: "Quit", value: NAV.QUIT } });
  let values = nav.map((c) => c.value);
  assert.ok(values.includes(NAV.NEXT));
  assert.ok(!values.includes(NAV.PREV));
  assert.ok(values.includes(NAV.QUIT));

  p.next();
  p.next();
  nav = navChoices(p, { exit: { label: "Back", value: NAV.BACK } });
  values = nav.map((c) => c.value);
  assert.ok(!values.includes(NAV.NEXT));
  assert.ok(values.includes(NAV.PREV));
  assert.ok(values.includes(NAV.BACK));
});
